...
class MentorDocumentsServiceGetTests : MentorDocumentsServiceMocks() {
    ...
    inner class GetDocsByUserIdTests {
        ...
        fun `Should throw BadRequestException if user role is not mentor`() {
            ...
            assertThat(exception.message).isEqualTo("У этого пользователя есть роль ${user.role}, а не ${UsersRoles.MENTOR}. Пожалуйста, используйте другой контроллер для проверки его документов.")
