...
class MentorDocumentsServiceGetTests : MentorDocumentsServiceMocks() {
    ...
    inner class GetDocsByUserIdTests {
        ...
        fun `Should throw BadRequestException if user role is not mentor`() {
            ...
            assertThat(exception.message).isEqualTo("У этого пользователя есть роль ${user.role}, а не ${UsersRoles.MENTOR}. Пожалуйста, используйте другого администратора для проверки его документов.")
