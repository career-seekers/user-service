...
import org.careerseekers.userservice.mocks.generators.DocumentsGenerator.createTutorDocumentsDto
import org.careerseekers.userservice.mocks.generators.UsersGenerator.createUser
import org.junit.jupiter.api.Nested
import kotlin.test.Test
import kotlin.test.assertFailsWith

class TutorDocumentsServiceCreateTests : TutorDocumentsServiceMocks() {

    @Nested
    inner class CreateTests {

        @Test
        fun `Should create tutor documents and return TutorDocuments`() {
            val user = createUser().copy(role = UsersRoles.TUTOR)
            val dto = createTutorDocumentsDto(user)
        ...
        @Test
        fun `Should throw BadRequestException when user exist but it's role not TUTOR`() {
